...
3. Собственный недельный расход текущей модели — второе значение сегмента
   `w:`, после серого `/` — в stdin вообще не приходит: в поле
   `rate_limits` от Claude Code нет разбивки по моделям. Скрипт определяет
   текущую модель из полей `model.display_name`/`model.id` на stdin (само
   имя модели нигде не отображается, оно используется только для этого
   сопоставления). Как только имя модели известно, скрипт запрашивает
   недельный расход из массива `limits[]` того же ответа
